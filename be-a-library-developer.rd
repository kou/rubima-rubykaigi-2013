...
では、この今いる場所とゴールがわかっている状態で間をどうやって埋めていけばよいのでしょうか。残念ながら私はこの問いに対する答えをまだ持っていません。それでも自分はどうやっているだろうかと思い出して説明しようとしてみると、まず、ゴールを手前に近づけようとしている気がします。探り探り感を出すためにASCIIアートで説明します。
...
この状態からゴールに到達するために、ゴールの少し手前に「ここからゴールまではなんとかなりそうな場所」を探します。
...
さて、みなさんは「今いる場所」と「ゴール」がわかっているとき、どうしているでしょうか。

だいぶ補足が長くなりましたが、実際に「想像するんじゃなくて思い出す」というキーとなる考えを適用してみるという練習をしました。Rubyistとして普通はどうやっているかを「思い出す」、そしてそれと同じようにする、ということがピンときましたか？

== まとめ

まとめます。この発表では「よいソフトウェアを開発する方法」を説明しました。より「よい」とは「Rubyらしい」、言い換えると「他と似ている」ということです。これを実現するためのキーとなる考えが「想像するよりも思い出す」です。なぜなら想像することは知らないことを閃かないといけないので難しく、思い出すことは知っていることから選ぶだけなので簡単だからです。ソフトウェア開発に当てはめてみると、思い出すためにはRubyistとしての経験が必要です。あとはその経験を思い出せばいいのです。しかし、「思い出す」という経験がないので「思い出す」ことが難しいことでしょう。「思い出す」経験をするためにはライブラリー開発者になることをオススメします。ライブラリーを開発すると何度も何度も「思い出す」必要があり、とてもよい練習になります。

== 著者紹介

須藤功平。フリーソフトウェアプログラマーで((<株式会社クリアコード|URL:http://www.clear-code.com/>))代表取締役（2 代目）。社名の命名者でもある。社名の由来は「クリアなコード」。その名の通りクリアなコードを書く会社であろうという意図を込めている。最近は自分たちのよいコードを書くノウハウを伝えるサービス「((<コミットへのコメントサービス|URL:http://www.clear-code.com/services/commit-comment.html>))」を始めた。サービスを受けたい開発チーム、クリアコードでこのサービスを担当したい開発者、どちらも募集中。
